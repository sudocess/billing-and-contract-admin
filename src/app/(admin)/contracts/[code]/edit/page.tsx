...
import ExtensionWizard, { type ExtensionPrefill } from '@/components/ExtensionWizard'
...
export default async function EditContractPage({
  ...
  const snapshot = (contract.data as PreviewData | null) ?? null
  const client = {
    ...
    country: contract.clientCountry ?? 'Netherlands',
  ...
  /* A scope extension has its own wizard, so it gets its own editor. Opening one in
     the project wizard offered a plan, a phase and a hosting arrangement it does not
     have, and previewed its payment rows as a 30/40/30 split it never agreed to. */
  if (contract.contractType === 'extension') {
    const extPrefill: ExtensionPrefill = {
      contractCode: contract.contractCode,
      extendsCode: snapshot?.extendsCode ?? '',
      ...
      client,
      schedule: parseSchedule(contract.installments) ?? parseSchedule(snapshot?.schedule) ?? null,
    }
    return (
      <>
        {header}
        <div className="p-4 sm:p-7 flex-1">
          <ExtensionWizard prefill={extPrefill} />
        </div>
      </>
    )
  }
